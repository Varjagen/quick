  ...
  switch (action.type) {
    ...
    case 'MOVEMENT_USE': {
      ...
      if (mv.jumpPending) {
        ...
        const base = (mv.preMode === 'fly') ? flySpeedOf(ent) : walkSpeedOf(ent);
    ...
    case 'MOVEMENT_MODE': {
      ...
      if (action.mode === 'dash') {
        const base = (mv.mode === 'fly') ? fly : walk;
...
function MapCanvas({
  ...
  const startTokenDrag = (tokenId, e) => {
    ...
    if (isMyActiveTurn && ent) {
      const speedFt = ent.speed > 0 ? ent.speed : 30;
      const usedFt = (movement && movement.entityId === token.entityId) ? (movement.usedFt || 0) : 0;
          ...
          <div className="move-range-group" style={{ opacity: moveRangeOpacity }}>
            ...
            {moveMarker.remR > 0 && (
              <div className="move-range move-range-rem" style={{
                ...
                <span className="move-range-label">{Math.round(moveMarker.remFt)} ft{moveMarker.mode === 'fly' ? ' · fly' : moveMarker.mode === 'jump' ? ' · jump' : moveMarker.dashed ? ' · dash' : ''}</span>
              </div>
            )}
          </div>
        )}
...
function PlayerActionRequest({ sourceOptions, defaultSourceId, targetToken, targetEntity, playerActionSender, dmMode = false, onDmSubmit, attackOnly = false, rangeInfo, physicalDice = false }) {
  ...
  const sendManeuver = (m) => {
    ...
    submit('apply_damage', {
      ...
      weaponName: m.label, attackName: '',
      effect: m.condition ? { condition: m.condition, save: { ability: m.ability, dc: Number(m.dc) || 10 } } : null,
    });
  ...
  return (
    <div className="par-box">
      ...
      {mode === 'maneuver' ? (
            ...
            return (
              <div className="par-man-detail">
                ...
                <button className="btn primary par-send" disabled={!reach}
                  onClick={() => sendManeuver({ label, condition: manCond, ability: manAbil, dc: manDc, range })}>
        ...
        <div className="par-conds">
          {CONDITIONS.map(c => (
            <button key={c} className="par-cond" style={{ borderColor: CONDITION_COLORS[c] || '#888' }}
              onClick={() => sendCondition(c)}>
              <span className="par-cond-dot" style={{ background: CONDITION_COLORS[c] || '#888' }} />{c}
            </button>
          ))}
        </div>
      ) : showWeapon ? (
...
function TokenDetailPanel({ state, token, entity, mode, dispatch, onClose, claimedEntityId, myPeerId, playerActionSender, onLongRest, onShortRest, obfuscateHp, physicalDice }) {
  ...
  const mvWalk = walkSpeedOf(entity), mvFly = flySpeedOf(entity), mvJump = jumpSpeedOf(entity);
        ...
        {isActiveMover && ownsThis && (
          <div className="move-actions" style={{ marginBottom: 10 }}>
            ...
            <div className="move-actions-rem">
              <b>{mvRemaining} ft</b> remaining{mvMode === 'jump' ? ' · jump armed (next move)' : mvDashed ? ' · dashing' : mvMode === 'fly' ? ' · flying' : ''}
            </div>
          </div>
        )}
          ...
          <div className="settings-section">
            ...
            <div className="toggle-row" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <label htmlFor="physical-dice-toggle" style={{ cursor: 'pointer', fontSize: 13 }}>
                Roll your own dice - type in each result instead of auto-rolling
              </label>
            </div>
            <div className="settings-hint">
              When on, attacks and saving throws let you enter the numbers you rolled at the table (the to-hit d20s, each damage die, and saves).
            </div>
          </div>
...
const MANEUVER_PRESETS = [
  { key: 'grapple', label: 'Grapple', condition: 'Grappled',   ability: 'STR', dc: 13, range: 5, icon: '🤼' },
  { key: 'shove',   label: 'Shove',   condition: 'Prone',      ability: 'STR', dc: 13, range: 5, icon: '👊' },
  { key: 'push',    label: 'Push',    condition: '',           ability: 'STR', dc: 13, range: 5, icon: '💪' },
  { key: 'trip',    label: 'Trip',    condition: 'Prone',      ability: 'DEX', dc: 13, range: 5, icon: '🦵' },
  { key: 'frighten',label: 'Frighten',condition: 'Frightened', ability: 'WIS', dc: 13, range: 30, icon: '😱' },
  { key: 'restrain',label: 'Restrain',condition: 'Restrained', ability: 'STR', dc: 13, range: 5, icon: '🕸' },
];
...
const WEAPON_PRESETS = [
  { name: 'Dagger',         atk: { name: 'Stab',   range: 5,   dmg: [[1, 4, 0, 'Piercing']] } },
  { name: 'Club',           atk: { name: 'Bash',   range: 5,   dmg: [[1, 4, 0, 'Bludgeoning']] } },
  { name: 'Handaxe',        atk: { name: 'Chop',   range: 5,   dmg: [[1, 6, 0, 'Slashing']] } },
  { name: 'Shortsword',     atk: { name: 'Thrust', range: 5,   dmg: [[1, 6, 0, 'Piercing']] } },
  { name: 'Mace',           atk: { name: 'Swing',  range: 5,   dmg: [[1, 6, 0, 'Bludgeoning']] } },
  { name: 'Spear',          atk: { name: 'Jab',    range: 5,   dmg: [[1, 6, 0, 'Piercing']] } },
  { name: 'Quarterstaff',   atk: { name: 'Strike', range: 5,   dmg: [[1, 6, 0, 'Bludgeoning']] } },
  { name: 'Rapier',         atk: { name: 'Lunge',  range: 5,   dmg: [[1, 8, 0, 'Piercing']] } },
  { name: 'Longsword',      atk: { name: 'Slash',  range: 5,   dmg: [[1, 8, 0, 'Slashing']] } },
  { name: 'Battleaxe',      atk: { name: 'Cleave', range: 5,   dmg: [[1, 8, 0, 'Slashing']] } },
  { name: 'Warhammer',      atk: { name: 'Smash',  range: 5,   dmg: [[1, 8, 0, 'Bludgeoning']] } },
  { name: 'Greatsword',     atk: { name: 'Hew',    range: 5,   dmg: [[2, 6, 0, 'Slashing']] } },
  { name: 'Greataxe',       atk: { name: 'Cleave', range: 5,   dmg: [[1, 12, 0, 'Slashing']] } },
  { name: 'Maul',           atk: { name: 'Crush',  range: 5,   dmg: [[2, 6, 0, 'Bludgeoning']] } },
  { name: 'Halberd',        atk: { name: 'Reach',  range: 10,  dmg: [[1, 10, 0, 'Slashing']] } },
  { name: 'Shortbow',       atk: { name: 'Shoot',  range: 80,  dmg: [[1, 6, 0, 'Piercing']] } },
  { name: 'Longbow',        atk: { name: 'Shoot',  range: 150, dmg: [[1, 8, 0, 'Piercing']] } },
  { name: 'Light Crossbow', atk: { name: 'Shoot',  range: 80,  dmg: [[1, 8, 0, 'Piercing']] } },
  { name: 'Sling',          atk: { name: 'Sling',  range: 30,  dmg: [[1, 4, 0, 'Bludgeoning']] } },
];
function weaponFromPreset(p) {
  return {
    id: uid('wpn_'), name: p.name, equipped: true,
    attacks: [{
      id: uid('atk_'), name: p.atk.name, toHit: 0, range: p.atk.range,
      damage: p.atk.dmg.map(([count, sides, modifier, type]) => ({ count, sides, modifier, type })),
    }],
  };
}
...
function WeaponManager({ initial, onChange, canEdit, abilities }) {
  ...
  const addWeapon = () => {
    const w = { id: uid('wpn_'), name: 'New Weapon', equipped: true, attacks: [newWeaponAttack()] };
    setWeapons(ws => [...ws, w]); setOpenId(w.id);
  };
  const addPreset = (name) => {
    const p = WEAPON_PRESETS.find(x => x.name === name);
    if (p) setWeapons(ws => [...ws, weaponFromPreset(p)]);
  };
          ...
          <div key={w.id} className={`wpn-card ${w.equipped ? 'equipped' : ''}`}>
            <div className="wpn-card-head">
              <button className={`wpn-eq-toggle ${w.equipped ? 'on' : ''}`} title={w.equipped ? 'Equipped' : 'Not equipped'}
                onClick={() => patchWeapon(w.id, { equipped: !w.equipped })}>{w.equipped ? '★ Equipped' : '☆ Unequipped'}</button>
      ...
      <div className="csheet-body">
        ...
        {tab === 'gear' && (
          <>
            ...
            <WeaponManager key={entity.id} initial={entity.weapons || []} canEdit={canEdit} abilities={entity.abilities} onChange={(w) => onField({ weapons: w })} />
...
function NewCharacterBuilder({ playerName, busy, onCancel, onCreate, onRoll }) {
  ...
  useEffect(() => { saveCreation({ phase, name, race, level, pool, assign, cls, classLocked, hp, hbSpeed, hbSize, hbHeight, hbTraits }); },
    [phase, name, race, level, pool, assign, cls, classLocked, hp, hbSpeed, hbSize, hbHeight, hbTraits]);
  ...
  const finish = () => {
    ...
    onCreate({
      name: name.trim() || `${race} ${cls}`, race, class: cls, level, stats, hp,
  ...
  const Stepper = (
    <div className="cc-steps">
      {['Class', 'Race', 'Stats', 'Health'].map((s, i) => (
      ...
      {phase === 3 && (
        <div className="cc-body">
          ...
          <div className="builder-actions">
            ...
            <button className="btn primary" disabled={!allRolled || assignedCount < 6} onClick={() => setPhase(4)}>Next: Health →</button>
          </div>
        </div>
      )}

      {/* Phase 4 - lock-in warning, then HP roll */}
      {phase === 4 && (
        <div className="cc-body">
          {!classLocked ? (
            <>
...
function AttackCinematic({ atk, isDM, onApply, onDismiss, onSetAdv, onSaveResult, physicalDice }) {
    ...
    <div className="acin-overlay">
      <div className={`acin-card ${isManeuver ? 'is-maneuver' : ''} ${pi >= 2 && hasToHit ? (hit ? 'is-hit' : 'is-miss') : ''}`}>
        <div className="acin-head">
          <div className={`acin-side ${isManeuver && pi >= 1 ? 'acin-clash-l' : ''}`}>
          ...
          <div className="acin-mid">
            <div className={`acin-swords ${isManeuver ? 'acin-man-glyph' : ''}`}>{isManeuver ? '🤼' : '⚔'}</div>
            {isManeuver && <div className="acin-man-tag">CONTEST</div>}
          ...
          <div className={`acin-side ${isManeuver && pi >= 1 ? 'acin-clash-r' : ''}`}>
        ...
        {hit && pi >= 3 && eff && eff.condition && (
          <div className="acin-effect">
            {eff.save ? (
              <>
                <div className="acin-effect-title">⚠ {eff.condition} <span className="acin-effect-sub">{eff.save.ability} save DC {eff.save.dc}</span></div>
                {!sr ? (isDM ? (
                  <div className="acin-save-roll">
                    ...
                    {physicalDice ? (
                      saveEntry ? (
                        ...
                        <button className="btn sm primary" onClick={() => setSaveEntry({ a: '', b: '' })}>✎ Enter {eff.save.ability} save</button>
                      )
                    ) : (
                      <button className="btn sm primary" onClick={rollSave}>🎲 Roll {eff.save.ability} save</button>
                    )}
                  </div>
                ) : <div className="acin-await">Awaiting the save…</div>) : (
                  <div className={`acin-save-result ${sr.success ? 'pass' : 'fail'}`}>
                    {sr.ability} save: {sr.roll}{sr.mod >= 0 ? ' + ' : ' - '}{Math.abs(sr.mod)} = <b key={sr.total} className="acin-glow-num">{sr.total}</b> vs DC {eff.save.dc}
                    <span className="acin-save-verdict">{sr.success ? '✦ Resisted' : `✕ ${eff.condition}!`}</span>
                  </div>
                )}
              </>
            ) : (
...
function AttackCinematicLayer({ state, dispatch, isDM, toast, physicalDice }) {
  ...
  return ReactDOM.createPortal(
    <AttackCinematic key={atk.id} atk={atk} isDM={isDM} physicalDice={physicalDice}
      onApply={isDM ? onApply : null} onDismiss={isDM ? onDismiss : null}
      onSetAdv={isDM ? onSetAdv : null} onSaveResult={isDM ? onSaveResult : null} />,
    document.body
  );
}
...
function PlayerInterface({ state, dispatch, myPeerId, playerName, sync, syncStatus, onLogout, roomCode, toast, settings, onSettingsChange, onOpenSettings, showSettings, onCloseSettings }) {
  ...
  return (
    <div className={`app-shell ${sicknessWobbleClass}`}>
      ...
      <AttackCinematicLayer state={state} dispatch={() => {}} isDM={false} physicalDice={settings?.physicalDice === true} />
    </div>
  );
}
  ...
  const handlePlayerAction = useCallback((action, peerId) => {
    ...
    switch (action.type) {
      ...
      case 'submit_request': {
        ...
        const ACTION_KINDS = ['apply_damage', 'apply_heal', 'apply_condition'];
        if (!['new_character', 'stat_change', 'level_change', ...ACTION_KINDS].includes(kind)) return;
        ...
        if (ACTION_KINDS.includes(kind)) {
          ...
          if (kind === 'apply_condition') {
            ...
            if (e0 && typeof e0 === 'object' && CONDITIONS.includes(e0.condition)) {
              ...
              effect = { condition: e0.condition, save };
            }
      ...
      case 'move_token': {
        ...
        if (curr.lockOffTurn && curr.initiative?.active && curr.initiative.entries.length) {
          ...
          if (activeId !== entity.id) {
            console.log(`[plagues-call] move_token REJECT: off-turn locked (active=${String(activeId).slice(-6)}, tried=${entity.id.slice(-6)})`);
        ...
        console.log(`[plagues-call] DM move_token OK peer=${peerId.slice(-6)} token=${tokenId.slice(-6)} → (${nx.toFixed(0)}, ${ny.toFixed(0)})`);
